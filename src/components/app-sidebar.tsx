...
export function AppSidebar() {
  ...
  return <Sidebar className={`bg-[#1A1F2C] text-white ${collapsed ? "w-16" : "w-56"} transition-all duration-250`}>
      <SidebarContent className="bg-slate-900 rounded-none">
        <SidebarGroup>
          <div className={`flex items-center justify-center my-4 ${collapsed ? "px-0" : "px-2"}`}>
            <Link to="/dashboard">
              {/* Use image logo only if not collapsed */}
              {!collapsed ? <LogoMarvellous className="h-12 w-auto mx-auto" /> : <LogoMarvellous className="h-8 w-auto mx-auto" minimal />}
            </Link>
          </div>
        </SidebarGroup>
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu>
              ...
              {currentUser?.isAdmin && adminItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={location.pathname.startsWith(item.url)}>
                    <Link to={item.url}>
                      <item.icon className="mr-0" />
                      {!collapsed && <span className="ml-2">{item.title}</span>}
      ...
      <SidebarFooter>
        {collapsed ? null : <span className="text-[10px] text-center opacity-60 block w-full">© Marvellous Manager 2025</span>}
      </SidebarFooter>
    </Sidebar>;
}