...
const PrintPage = () => {
  ...
  const [confirmProjectUnavailable, setConfirmProjectUnavailable] = useState(false);
  ...
  const handlePrint = async () => {
    // Check if user has confirmed project unavailability for all-hards print
    if (printType === "all-hards" && !confirmProjectUnavailable) {
      alert("Please confirm that you understand this action will mark the project as unavailable.");
      return;
    }
   ...
   .content-wrapper{
      ...
      if (printType === "all-hards" && isProjectPrint && project) {
        toast.success(`Print completed successfully! Project "${project.name}" has been marked as unavailable.`);
      } else {
        toast.success("Print completed successfully!");
      }
    } catch (error) {
      toast.error("Failed to complete print operation. Please try again.");
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* Project Unavailability Warning for All-Hards Print */}
            {printType === "all-hards" && isProjectPrint && project && (
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  <div className="space-y-2">
                    <p className="font-medium">Important: This action will mark the project "{project.name}" as unavailable.</p>
                    <p className="text-sm text-muted-foreground">
                      After printing all hard drives for this project, the project status will be automatically set to "unavailable" 
                      to indicate that all hard drives have been processed and the project is no longer active.
                    </p>
                    <div className="flex items-center space-x-2 mt-3">
                      <Checkbox 
                        id="confirmProjectUnavailable" 
                        checked={confirmProjectUnavailable}
                        onCheckedChange={(checked) => setConfirmProjectUnavailable(checked as boolean)}
                      />
                      <Label htmlFor="confirmProjectUnavailable" className="text-sm">
                        I understand that this will mark the project as unavailable
                      </Label>
                    </div>
                  </div>
                </AlertDescription>
            ...
            <Button 
              ...
              disabled={!operatorName || (printType === "all-hards" && !confirmProjectUnavailable)}
              onClick={handlePrint}
            >
